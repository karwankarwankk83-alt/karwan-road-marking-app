...
function thicknessControl(){const p=PAINT[$('#cPaint')?.value]||PAINT.thermo;return `<section class="c3-thickness"><div><b>ئەستووری بۆیاخ بە پێی پڕۆژە</b><small>ئەستووری دیاریکراو لە Specification یان TDS بنووسە</small></div><div class="c3-thickness-control"><button type="button" onclick="stepThickness(-.1)">−</button><input id="cThickness" type="number" min="0" step=".1" value="${p.thickness}" inputmode="decimal"><em>mm</em><button type="button" onclick="stepThickness(.1)">＋</button></div></section>`}
window.stepThickness=function(change){const el=$('#cThickness');if(!el)return;el.value=Math.max(0,Math.round((n('cThickness')+change)*10)/10);el.dispatchEvent(new Event('input',{bubbles:true}))};
function shell(title,help,fields){
 return `<div class="c3-top"><button class="c3-back" type="button" onclick="calcTool()">‹ گەڕانەوە</button><div><b>${title}</b><small>${help}</small></div></div><div class="c3-form"><div class="c3-grid">${fields}</div>${thicknessControl()}${settings()}</div><div id="c3Results" class="c3-results"></div>`;
}
...
window.renderCalc=function(type){
 ...
 c.innerHTML=`<div class="c3-top"><button class="c3-back" type="button" onclick="calcTool()">‹ گەڕانەوە</button><div><b>تێکەڵکردنی 2K</b><small>ڕێژەی هاردنەر لە TDS ـی بەرهەمەکەت وەربگرە</small></div></div><div class="c3-form"><div class="c3-grid">${input('بڕی ماددەی بنەڕەت','baseKg',100,'kg','.1')}${input('ڕێژەی هاردنەر لە ماددەی بنەڕەت','hardPercent',2,'%','.1')}</div><p class="c3-mix-help"><b>نموونە:</b> 100 kg ماددەی بنەڕەت + 2% هاردنەر = 2 kg هاردنەر؛ کۆی تێکەڵەکە دەبێتە 102 kg.</p></div><div id="c3Results" class="c3-results"></div>`;
 bind(()=>{const base=n('baseKg'),percent=n('hardPercent'),hard=base*percent/100,total=base+hard;$('#c3Results').innerHTML=`<h3>ئەنجامی تێکەڵ</h3><div class="c3-main-result"><span>کۆی تێکەڵ</span><strong>${total.toFixed(2)}</strong><b>kg</b></div><div class="c3-result-grid"><div><span>ماددەی بنەڕەت</span><b>${base.toFixed(2)} kg</b></div><div><span>هاردنەر (${percent.toFixed(2)}%)</span><b>${hard.toFixed(2)} kg</b></div></div><p>ڕێژەی هاردنەر سەدی ماددەی بنەڕەتە، نەک سەدی کۆی تێکەڵ. هەمیشە TDS ـی بەرهەمەکەت بپشکنە.</p>`});
};
...
.c3-thickness{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:12px;padding:13px;border-radius:15px;background:#292500;border:1px solid #6a5908}.c3-thickness b,.c3-thickness small{display:block}.c3-thickness b{font-size:15px;color:var(--yellow)}.c3-thickness small{font-size:12px;color:#c9c3a8;margin-top:4px;line-height:1.5}.c3-thickness-control{display:grid;grid-template-columns:42px 72px auto 42px;align-items:center;gap:5px;direction:ltr;flex:0 0 auto}.c3-thickness-control button{height:42px;border:0;border-radius:11px;background:var(--yellow);color:#111;font-size:20px;font-weight:900}.c3-thickness-control input{width:72px;height:42px;text-align:center;border:1px solid #6a5908;border-radius:10px;background:#0d0f0f;color:#fff;font-size:18px}.c3-thickness-control em{font-style:normal;font-size:13px;color:#ddd}.c3-advanced{margin-top:10px;border-top:1px solid var(--line);padding-top:10px}.c3-advanced summary{cursor:pointer;color:var(--yellow);font-size:14px;font-weight:800;padding:7px}.c3-advanced p{font-size:12px;color:var(--muted);line-height:1.7;margin:8px}
.c3-mix-help{grid-column:1/-1;margin:10px 2px 0;padding:11px;border-radius:12px;background:#242109;color:#ddd;font-size:13px;line-height:1.8}.c3-mix-help b{color:var(--yellow)}
...
@media(max-width:650px){.c3-thickness{display:block}.c3-thickness-control{margin-top:11px;grid-template-columns:48px 1fr auto 48px}.c3-thickness-control input{width:100%}.c3-choices{grid-template-columns:1fr 1fr}.c3-grid,.c3-subgrid{grid-template-columns:1fr}.c3-subgrid{grid-column:auto}.c3-field{padding:10px}.c3-result-grid{grid-template-columns:1fr 1fr}}
