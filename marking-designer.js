...
const templates={
 stop:{name:'STOP / وەستان',kind:'text',text:'STOP',w:2.4,h:1.6,note:'نووسینی سەر شەقام؛ قەبارەکە بەپێی خێرایی دەگۆڕێت.'},
 ...
 straight:{name:'تیری ڕاستەوخۆ',kind:'arrow',dir:'up',w:1.25,h:5,note:'614.1؛ درێژی گشتی 5.00 m.'},
 right:{name:'تیری لای ڕاست',kind:'arrow',dir:'right',w:1.8,h:5,note:'614.2؛ درێژی گشتی 5.00 m.'},
 left:{name:'تیری لای چەپ',kind:'arrow',dir:'left',w:1.8,h:5,note:'614.3؛ درێژی گشتی 5.00 m.'},
...
function drawing(t,w,h){
 ...
 return `<svg id="mdSvg" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg"><rect width="300" height="300" rx="18" fill="#202322"/><g fill="#fff">${body}</g><g stroke="#f4c400" stroke-width="2" fill="none"><path d="M15 275H285M15 269v12M285 269v12"/><path d="M8 20V250M2 20h12M2 250h12"/></g><g fill="#f4c400" font-size="13" font-family="Arial"><text x="150" y="294" text-anchor="middle">${w.toFixed(2)} m</text><text x="10" y="140" text-anchor="middle" transform="rotate(-90 10 140)">${h.toFixed(2)} m</text></g></svg>`;
}
...
function renderResult(){
 ...
 if(active==='custom'){t.text=$('#mdText').value.trim()||'KS'}
 const stripe=t.kind==='zebra'?`<div><span>پانی هەر هێڵ</span><b>${((t.stripe||.5)*100).toFixed(0)} cm</b></div>`:'';
 $('#mdResult').innerHTML=`<div class="md-preview">${drawing(t,w,h)}</div><div class="md-data"><h3>${t.name}</h3><div class="md-measures"><div><span>پانی گشتی</span><b>${w.toFixed(2)} m</b></div><div><span>درێژی گشتی</span><b>${h.toFixed(2)} m</b></div>${stripe}<div><span>خێرایی</span><b>${speed.toFixed(0)} km/h</b></div></div><p>${t.note}</p><p class="md-source">DTCDM 2nd Edition (2015), Volume 2 — Chapter 6</p><button class="md-download" onclick="downloadMarkingSvg()">⇩ دابەزاندنی نەخشە SVG</button></div>`;
}
function select(key){
 active=key;const t=templates[key];speedPreset(t,num('mdSpeed')||60);
 $('#mdW').value=t.w;$('#mdH').value=t.h;$('#mdCustom').hidden=key!=='custom';if(key==='custom')$('#mdText').value=t.text;
 document.querySelectorAll('.md-chip').forEach(x=>x.classList.toggle('active',x.dataset.key===key));renderResult();
}
window.markingDesignerView=function(){
 const v=$('#view');if(!v)return;
 v.innerHTML=`<section class="md-head"><button onclick="homeView()">‹ گەڕانەوە</button><div><b>دروستکەری نەخشەی مەیدانی</b><small>پێوانەکانی Dubai DTCDM</small></div></section><section class="md-search"><label>ناوی هێما یان نیشانە بنووسە</label><input id="mdSearch" placeholder="STOP، زێبرا، تیر، پاسکیل…" inputmode="search"><div class="md-chips">${Object.entries(templates).map(([k,t])=>`<button class="md-chip" data-key="${k}">${t.name}</button>`).join('')}</div></section><section class="md-controls"><label>خێرایی شەقام <div><input id="mdSpeed" type="number" value="60" min="0" step="10"><em>km/h</em></div></label><label>پانی گشتی <div><input id="mdW" type="number" min="0" step=".01"><em>m</em></div></label><label>درێژی گشتی <div><input id="mdH" type="number" min="0" step=".01"><em>m</em></div></label><label id="mdCustom" hidden>نووسینی تایبەت <div><input id="mdText" value="KS"><em>ABC</em></div></label></section><section id="mdResult" class="md-result"></section><p class="md-warning">تێبینی: پێش جێبەجێکردن نەخشە و Specification ـی پڕۆژە و ڕەزامەندی ئەندازیاری چاودێر پشتڕاست بکەرەوە.</p>`;
 document.querySelectorAll('.md-chip').forEach(b=>b.onclick=()=>select(b.dataset.key));
 $('#mdSearch').addEventListener('input',e=>{const q=e.target.value.toLowerCase();const hit=Object.entries(templates).find(([k,t])=>t.name.toLowerCase().includes(q)||k.includes(q)||(q.includes('ستۆپ')&&k==='stop')||(q.includes('زێبرا')&&k==='zebra')||(q.includes('پێداویستی')&&k==='disabled'));if(hit)select(hit[0])});
 $('#mdSpeed').addEventListener('change',()=>select(active));['mdW','mdH','mdText'].forEach(id=>$('#'+id)?.addEventListener('input',renderResult));select('stop');
};
