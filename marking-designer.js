...
function drawing(t,w,h){
 ...
 if(t.kind==='text'){
  ...
  textDims=`<g stroke="#59615d" stroke-width=".55" opacity=".72" fill="none">${grid}</g><g stroke="#58d68d" stroke-width="1.3" fill="none">${top.replace(/<text[\s\S]*?<\/text>/g,'')}${right.replace(/<text[\s\S]*?<\/text>/g,'')}${stroke.replace(/<text[\s\S]*?<\/text>/g,'')}</g><g fill="#58d68d" font-size="7.5" font-family="Arial">${top.match(/<text[\s\S]*?<\/text>/g)?.join('')||''}${right.match(/<text[\s\S]*?<\/text>/g)?.join('')||''}${stroke.match(/<text[\s\S]*?<\/text>/g)?.join('')||''}<text x="150" y="254" text-anchor="middle">گرید ${Math.round(m.cell*100)} cm × ${Math.round(m.cell*100)} cm</text></g>`;
 }
 ...
 const universalDims=`<g stroke="#f4c400" stroke-width="2" fill="none"><path d="M35 10H265M35 5v10M265 5v10"/><path d="M292 30V240M287 30h10M287 240h10"/><path d="M15 275H285M15 269v12M285 269v12"/><path d="M8 20V250M2 20h12M2 250h12"/></g><g fill="#f4c400" font-size="10" font-family="Arial" direction="ltr"><text x="150" y="9" text-anchor="middle">${w.toFixed(2)} m</text><text x="299" y="135" text-anchor="middle" transform="rotate(90 299 135)">${h.toFixed(2)} m</text><text x="150" y="294" text-anchor="middle">${w.toFixed(2)} m</text><text x="10" y="140" text-anchor="middle" transform="rotate(-90 10 140)">${h.toFixed(2)} m</text></g>`;
