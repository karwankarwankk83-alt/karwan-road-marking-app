(()=>{
  ...
  const css=`
  .ks-ai-btn{position:fixed;left:-7px;right:auto;top:34%;bottom:auto;z-index:90;width:45px;height:52px;border-radius:0 22px 22px 0;border:2px solid #806600;border-left:0;background:#f4c400;color:#111;font-size:21px;box-shadow:0 8px 24px #0008;cursor:pointer;opacity:.88;transition:left .2s,opacity .2s,transform .2s}.ks-ai-btn:hover,.ks-ai-btn:focus-visible{left:0;opacity:1;transform:scale(1.04)}
  ...
  const btn=document.createElement('button');btn.className='ks-ai-btn';btn.type='button';btn.title='یاریدەدەری کوردی';btn.setAttribute('aria-label','کردنەوەی یاریدەدەری کوردی');btn.textContent='👀';
  const panel=document.createElement('section');panel.className='ks-ai-panel';panel.innerHTML=`<div class="ks-ai-head"><div class="ks-ai-avatar">KS</div><div class="ks-ai-title"><b>یاریدەدەری KS</b><small>● ئامادەی یارمەتیدانم</small></div><button class="ks-ai-voice" type="button" aria-label="دەنگ">🔊</button><button class="ks-ai-close" type="button" aria-label="داخستن">×</button></div><div class="ks-ai-msgs"></div><form class="ks-ai-compose"><input class="ks-ai-input" autocomplete="off" placeholder="بە کوردی پرسیار بکە…"><button class="ks-ai-mic" type="button" aria-label="قسەکردن بە دەنگ">🎙️</button><button class="ks-ai-send" type="submit">➤</button></form><div class="ks-ai-note">یاریدەدەری AI ـی KS Road Marking</div>`;
  document.body.append(btn,panel);
  const msgs=panel.querySelector('.ks-ai-msgs'),input=panel.querySelector('.ks-ai-input'),form=panel.querySelector('form'),voiceBtn=panel.querySelector('.ks-ai-voice'),micBtn=panel.querySelector('.ks-ai-mic');
  ...
  if(SpeechRecognition){
    recognition=new SpeechRecognition();recognition.lang='ckb-IQ';recognition.interimResults=true;recognition.continuous=false;
    recognition.onstart=()=>{micBtn.classList.add('listening');micBtn.textContent='⏹️';input.placeholder='قسە بکە…'};
    recognition.onresult=e=>{let said='';for(let i=e.resultIndex;i<e.results.length;i++)said+=e.results[i][0].transcript;input.value=said};
    recognition.onend=()=>{micBtn.classList.remove('listening');micBtn.textContent='🎙️';input.placeholder='بە کوردی پرسیار بکە…';input.focus()};
    recognition.onerror=e=>{micBtn.classList.remove('listening');micBtn.textContent='🎙️';if(e.error==='not-allowed')add('assistant','تکایە ڕێگە بە میکرۆفۆن بدە، پاشان دووبارە هەوڵ بدە.');else add('assistant','دەنگەکەت تێنەگەیشتم؛ تکایە دووبارە بە هێواشی قسە بکە.')};
    micBtn.addEventListener('click',()=>{try{if(micBtn.classList.contains('listening'))recognition.stop();else{input.value='';recognition.start()}}catch(e){}});
  }else{micBtn.addEventListener('click',()=>add('assistant','براوزەرەکەت قسەکردن بە دەنگ پشتگیری ناکات. تکایە Google Chrome بەکاربهێنە.'))}
